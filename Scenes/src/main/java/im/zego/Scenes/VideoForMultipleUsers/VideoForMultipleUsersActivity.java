    ...
    public void loginRoom() {
        ...
        engine.loginRoom(roomID, user, RoomConfig, new IZegoRoomLoginCallback() {
            ...
            public void onRoomLoginResult(int errorCode, JSONObject extendedData) {
                AppLogger.getInstance().callApi("Login Room:%s",errorCode == 0?"succeed":"failed");
                if (errorCode != 0) {
                    Toast.makeText(VideoForMultipleUsersActivity.this, String.format("Login Room failed errorCode: %d", errorCode), Toast.LENGTH_SHORT).show();
                    finish();
                }
            }
        });
