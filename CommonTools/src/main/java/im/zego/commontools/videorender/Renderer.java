...
import im.zego.commontools.ve_gl.GlShader;
...
public class Renderer implements TextureView.SurfaceTextureListener {
    ...
    String streamID;
    ZegoPublishChannel publishChannel;
    ...
    public void setPublishChannel(ZegoPublishChannel channel){
    ...
    public void draw(VideoRenderHandler.PixelBuffer pixelBuffer) {
        ...
        if (pixelBuffer.strides[2] > 0) {
            ...
            int[] value = measure(pixelBuffer.width, pixelBuffer.height, viewWidth, viewHeight);
            // Render RGB image
            rgbDrawer.drawRgb(textureId, flipMatrix, pixelBuffer.width, pixelBuffer.height, value[0], value[1], value[2], value[3], pixelBuffer.flip);
            // swap rendered buffer to display
            swapBuffers();
            // detach current eglContext
            detachCurrent();
...
//    Calculate the actual image width and height and plane coordinates after zooming according to the image data and the width and height of the display view
    ...
    // attach the SurfaceView to EGL Surface
...
//    public native void copyPlane(ByteBuffer src, int srcStride, ByteBuffer dst, int dstStride, int width, int height);
    public static void copyPlane(ByteBuffer src, int srcStride, ByteBuffer dst, int dstStride, int width, int height){
        ...
        if(srcStride == dstStride){
            pos = src.position()+arrayoffset;
            remaining = dst.remaining();
            if(remaining < srcStride*height + arrayoffset){
                return;
            }

            dst.put(src.array(),pos, srcStride*height);
            dst.flip();
            return;
        }
        for (int i = 0; i < height; i++) {
            ...
            pos = src.position()+arrayoffset;
            remaining = dst.remaining();
            if(remaining < width + arrayoffset){
                return;
            }
            dst.put(src.array(),pos, width);
        }
        dst.flip();
