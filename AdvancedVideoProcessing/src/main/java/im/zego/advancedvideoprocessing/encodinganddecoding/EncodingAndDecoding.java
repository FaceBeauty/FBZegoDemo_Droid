...
public class EncodingAndDecoding extends AppCompatActivity {
    ...
    EditText editVideoGOP;
    Button videoConfigButton;

    EditText editsmallEncodeWidth;
    EditText editsmallEncodeHeight;
    EditText editsmallVideoFPS;
    EditText editsmallVideoBitrate;

    SwitchMaterial hardwareEncoderSwitch;
    SwitchMaterial hardwareDecoderSwitch;
    SwitchMaterial renderBackendSwitch;
    ...
    public void bindView(){
        ...
        editVideoGOP = findViewById(R.id.videoGOP);
        videoConfigButton = findViewById(R.id.videoConfigUpdate);

        editsmallEncodeWidth = findViewById(R.id.smallencodeResolutionWidth);
        editsmallEncodeHeight = findViewById(R.id.smallencodeResolutionHeight);
        editsmallVideoFPS = findViewById(R.id.smallvideoFps);
        editsmallVideoBitrate = findViewById(R.id.smallvideoBitrate);



        hardwareDecoderSwitch = findViewById(R.id.hardwareDecodeSwitch);
        hardwareEncoderSwitch = findViewById(R.id.hardwareEncodeSwitch);
        renderBackendSwitch = findViewById(R.id.renderBackendSwitch);
    ...
    public void setVideoConfigUpdateEvent() {
        videoConfigButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                videoConfig.keyFrameInterval = Integer.parseInt(editVideoGOP.getText().toString());

                engine.setVideoConfig(videoConfig);

                if(videoConfig.codecID != ZegoVideoCodecID.H264_DUAL_STREAM)
                    return;
                ...
                big.encodeWidth = Integer.parseInt(editEncodeWidth.getText().toString());
                big.encodeHeight = Integer.parseInt(editEncodeHeight.getText().toString());
                big.fps = Integer.parseInt(editVideoFPS.getText().toString());
                big.bitrate = Integer.parseInt(editVideoBitrate.getText().toString());
                config_list.add(big);

                ZegoPublishDualStreamConfig small = new ZegoPublishDualStreamConfig();
                small.streamType = ZegoVideoStreamType.SMALL;
                small.encodeWidth = Integer.parseInt(editsmallEncodeWidth.getText().toString());
                small.encodeHeight = Integer.parseInt(editsmallEncodeHeight.getText().toString());
                small.fps = Integer.parseInt(editsmallVideoFPS.getText().toString());
                small.bitrate = Integer.parseInt(editsmallVideoBitrate.getText().toString());
    ...
    public void setRenderBackendSwitchEvent(){
        renderBackendSwitch.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                ...
                if (isChecked){
                    config.advancedConfig.put("video_render_backend", "opengl3");
                    AppLogger.getInstance().callApi("Enable opengl3");
                } else {
                    config.advancedConfig.put("video_render_backend", "opengl2");
                    AppLogger.getInstance().callApi("Disable opengl3");
                }
