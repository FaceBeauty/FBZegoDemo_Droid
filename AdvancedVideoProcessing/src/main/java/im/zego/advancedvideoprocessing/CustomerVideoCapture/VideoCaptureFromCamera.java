...
/**
 *  * VideoCaptureFromCamera
 *  * To collect data from the camera and pass it to the ZEGO SDK.
...
//        restartCam();
    ...
    private int createCamOnCameraThread() {
        ...
        for (int i = 0; i < nCnt; i++) {
            ...
            if (mCamInfo.facing == nFacing) {
                try{
                    ...
                    Log.i(TAG, String.format("open camera %d failed", i));
                    mCam = null;
                }
        ...
        if (mCam == null) {
            ...
            try{
                ...
                Log.i(TAG, "[ERROR] open default camera failed");
                mCam = null;
            }
        ...
        Camera.Parameters parms = mCam.getParameters();
        // 获取camera支持的size
        // Get the camera's support size
        mWidth = 640;
        mHeight = 480;
        List<Camera.Size> psz = parms.getSupportedPreviewSizes();
        for (Camera.Size size: psz) {
    ...
    private int startCamOnCameraThread() {
        ...
        try {
            ...
            Log.e(TAG, "startPreviewOnCameraThread: Camera startPreview failed");
            e.printStackTrace();
        }
    ...
    public void onPreviewFrame(byte[] data, Camera camera) {
        ...
        Log.i(TAG, "onPreviewFrame: "+mWidth+"×"+mHeight);
        param.width = mWidth;
...
//        Log.i("testComp", "onSurfaceTextureAvailable:==== "+isRenderInit);
        if (!isRenderInit) {
            isRenderInit = FBEffect.shareInstance().initBufferRenderer(FBFormatEnum.FBFormatNV21,mWidth,mHeight, FBRotationEnum.FBRotationClockwise270,false,5);
            Log.i("testComp", "onSurfaceTextureAvailable:------- "+isRenderInit);
        }
        FBEffect.shareInstance().processBuffer(data);
        // 将采集的数据传给ZEGO SDK
        ...
        if (zegoExpressEngine != null) {
            zegoExpressEngine.sendCustomVideoCaptureRawData(byteBuffer, data.length, param, now, mChannel);
        }
        // 实现camera预览时的内存复用
