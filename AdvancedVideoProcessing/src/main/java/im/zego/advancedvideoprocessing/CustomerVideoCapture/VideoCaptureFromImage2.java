...
import im.zego.commontools.ve_gl.EglBase;
import im.zego.commontools.ve_gl.GlRectDrawer;
import im.zego.commontools.ve_gl.GlUtil;
import im.zego.zegoexpress.ZegoExpressEngine;
import im.zego.zegoexpress.constants.ZegoPublishChannel;

/**
 * VideoCaptureFromImage2
 * 实现将图片源作为视频数据并传给ZEGO SDK
 * 采用GL_TEXTURE_2D方式传递数据，即OpenGL ES的2d贴图
 */

/**
         * VideoCaptureFromImage2
         * To implement the image source as video data and pass it to the ZEGO SDK.
 * Use GL_TEXTURE_2D to transfer data, that is, 2D texture of OpenGL ES.
         */
    ...
    public final int init() {
        ...
        mHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                // Register the refresh callback of Choreographer to ensure that the picture will not be torn when drawing pictures later
    ...
    private void draw(final Bitmap bitmap) {
        try {
            ...
            if (mIsCapture.get()) {
                ...
                mSDKEngine.sendCustomVideoCaptureTextureData(mPreviewTextureId, mImageWidth, mImageHeight, now, mChannel);
            }
    ...
    private void releasePreviewSurface() {
        if (previewEglBase.hasSurface()) {
            try {
                ...
                // 此时previewEglBase中的eglSurface可能已经被别的地方释放了
                Log.e("VideoCaptureFromImage2",  String.format("previewEglBase.hasSurface: %s, exception： %s, ", previewEglBase.hasSurface()?"True":"False", exception.getMessage()));
            }
        }
    }
