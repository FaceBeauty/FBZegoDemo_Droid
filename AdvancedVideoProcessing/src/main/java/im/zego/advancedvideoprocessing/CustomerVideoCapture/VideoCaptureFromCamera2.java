...
import im.zego.advancedvideoprocessing.CustomerVideoCapture.ve_gl.EglBase;
import im.zego.advancedvideoprocessing.CustomerVideoCapture.ve_gl.EglBase14;
import im.zego.advancedvideoprocessing.CustomerVideoCapture.ve_gl.GlRectDrawer;
import im.zego.advancedvideoprocessing.CustomerVideoCapture.ve_gl.GlUtil;
...
public class VideoCaptureFromCamera2 extends ZegoVideoCaptureCallback implements
    ...
    public void setmFront(int mFront) {
        this.mFront = mFront;
    }
    ...
    private AtomicBoolean mIsCapture = new AtomicBoolean();
            ...
            cameraThreadHandler.post(new Runnable() {
                ...
                public void run() {
                    ...
                    if (mTextureView != null) {
                        if (mTextureView.getSurfaceTextureListener() != null && mTextureView.getSurfaceTextureListener().equals(VideoCaptureFromCamera2.this)) {
...
//        }
    ...
    private void draw(int textureId, int width, int height, float[] texMatrix) {
        ...
        try {
            ...
            if (!isRenderInit) {
                isRenderInit = FBEffect.shareInstance().initTextureRenderer(width, height,
                        FBRotationEnum.fromValue(0),
                        isFrontCamera, 5);
            }
            textureId = FBEffect.shareInstance().processTexture(textureId);
            ...
            if (mIsCapture.get()) {
                ...
                if (zegoExpressEngine != null) {
                    zegoExpressEngine.sendCustomVideoCaptureTextureData(textureId, width, height, now, mChannel);
                }
            }
    ...
    public void onFrameAvailable(SurfaceTexture surfaceTexture) {
        ...
        // Draw picture data
        // 绘制
        Log.d(TAG, "onFrameAvailable: mIsPreview"+mIsPreview);
        if (mIsPreview) {
            Log.d(TAG, "onFrameAvailable: 6666666666666666666");
