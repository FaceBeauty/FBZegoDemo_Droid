...
import com.nimo.fb_effect.adapter.FBEffectFilterItemViewBinder;
 ...
 * 特效滤镜
 */
public class FBEffectFilterFragment extends FBBaseLazyFragment {

  private View container;

  private RecyclerView rvFilter;

  private final List<FBEffectFilterConfig.FBEffectFilter> items = new ArrayList<>();
  ...
  @Override protected void initView(View view, Bundle savedInstanceState) {
    ...
    FBEffectFilterConfig filterList = FBConfigTools.getInstance().getEffectFilterConfig();

    if (filterList == null) {
      FBConfigTools.getInstance().getEffectFiltersConfig(new FBConfigCallBack<List<FBEffectFilterConfig.FBEffectFilter>>() {
        @Override public void success(List<FBEffectFilterConfig.FBEffectFilter> list) {
          items.addAll(list);
          initRecyclerView();
        }
  ...
  public void initRecyclerView() {
    listAdapter.register(FBEffectFilterConfig.FBEffectFilter.class, new FBEffectFilterItemViewBinder());
    listAdapter.setItems(items);
    rvFilter.setAdapter(listAdapter);
    rvFilter.smoothScrollToPosition(FBUICacheUtils.getEffectFilterPosition());
    //sync progress
...
//      container.getLayoutParams().height = 370;
  ...
  @Override protected void onFragmentStartLazy() {
    ...
    FBState.currentSliderViewState = FBViewState.FILTER_EFFECT;
    //同步滑动条
    RxBus.get().post(FBEventAction.ACTION_SYNC_PROGRESS, "");
  ...
  public void onDestroy() {
    ...
    FBEffect.shareInstance().setFilter(FBFilterEnum.FBFilterEffect.getValue(),"0");
  }
}
